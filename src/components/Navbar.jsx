        ...
        <div className={`flex justify-between items-center py-4 ${level === 0 ? "border-b border-slate-50" : ""}`}>
          ...
          {node.children?.length > 0 && (
            ...
            >
              <ChevronRight
                size={18}
                className={`transition-transform duration-300 text-slate-400 ${activeMobileSubs[node.id] ? "rotate-90" : ""}`}
              />
            </button>
          )}
        </div>
      ...
      <nav className={`fixed top-0 left-0 w-full z-[200] transition-all duration-300 ${
        ...
        <div className="w-full h-16 lg:h-20 flex items-center">
          <div className="max-w-7xl mx-auto px-6 w-full flex items-center justify-between h-full">
            ...
            {/* Iconos derecha */}
            <div className="flex items-center gap-3 md:gap-4 relative z-[210]">
              ...
              {isAuthenticated && user ? (
                <div className="hidden md:flex items-center gap-3 pl-4 border-l border-slate-200">
                  {/* ── Mi Perfil ── */}
                  <Link to="/perfil" className="flex flex-col items-end hover:opacity-70 transition-opacity">
                    <span className="text-[10px] font-black uppercase text-blue-600 leading-none">Mi perfil</span>
                ...
                <Link
                  to="/auth"
                  className="p-2 text-slate-700 hover:bg-slate-100 rounded-full transition-colors hidden md:block"
                >
                  <User size={20} />
                </Link>
              )}

              {/* Carrito — muestra conteo real */}
        ...
        <div className={`fixed inset-0 bg-white z-[190] md:hidden transition-transform duration-500 ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
          <div className="flex flex-col h-full pt-24 px-6 pb-10 overflow-y-auto">
            ...
            <div className="mb-6">
              {isAuthenticated ? (
                <div className="flex items-center gap-4 p-5 rounded-3xl bg-slate-50 border border-slate-100">
                  <div className="w-12 h-12 bg-blue-600 text-white rounded-full flex items-center justify-center font-black text-lg">
                    {user?.name?.[0]?.toUpperCase() || "U"}
                  </div>
                  <div className="flex-1 min-w-0">
                    <span className="block text-sm font-black uppercase italic truncate">{user?.name}</span>
            ...
            {isAuthenticated && (
              <Link
                to="/perfil"
                className="flex items-center gap-4 p-4 mb-4 rounded-2xl bg-blue-50 border border-blue-100"
              >
                <User size={18} className="text-blue-600" />
                <span className="font-black uppercase text-sm text-blue-700">Mi Perfil & Pedidos</span>
              </Link>
            )}
