...
export default function ProductDetail({ cart, toggleCart }) {
  ...
  if (!product) return <div className="p-20 text-center font-bold italic">PRODUCTO NO ENCONTRADO</div>;
              ...
              {images.map((img, idx) => (
                <button
                  ...
                  className={`min-w-[80px] h-[80px] rounded-2xl overflow-hidden border-2 transition-all ${
                    activeImg === img ? "border-slate-900 scale-95" : "border-transparent opacity-40 hover:opacity-100"
                  }`}
                >
                ...
                {hasDiscount && (
                  <div className="flex flex-col">
                    ...
                    <span className="text-[11px] font-black text-red-600 flex items-center gap-1 mt-1 bg-red-50 px-2 py-0.5 rounded-md w-fit">
                      <Tag size={10} strokeWidth={3} /> AHORRAS ${discountAmount.toLocaleString()}
                    </span>
                  </div>
                )}
              </div>
