...
const ProductCard = memo(({ p, isInCart, onToggle }) => {
  ...
  // Productos con variantes siempre van al detalle para elegir la variante
  ...
  return (
    <div className="group relative flex flex-col">
      ...
      {hasVariants && (
        <div className="absolute top-5 right-5 z-20 bg-slate-900/75 backdrop-blur-md text-white px-2.5 py-1 rounded-xl text-[9px] font-black tracking-wider">
          + opciones
        </div>
      )}
      ...
      <Link
        ...
        className="relative block overflow-hidden rounded-[2.5rem] bg-[#F5F5F7] aspect-[4/5] transition-all duration-500 group-hover:shadow-2xl group-hover:shadow-slate-200"
      >
        <img
      ...
      {/* Botón carrito:
          - Con variantes → va al detalle (no se puede añadir sin elegir variante)
          - Sin variantes → toggle directo */}
      {hasVariants ? (
        <Link
          to={`/productos/detalle/${p.id}`}
          className="absolute bottom-32 right-6 z-20 p-4 rounded-full bg-white text-slate-900 border border-slate-100 shadow-xl hover:bg-blue-600 hover:text-white transition-all duration-300"
          title="Ver opciones"
        >
        ...
        <div className="flex items-baseline gap-3">
          {hasVariants ? (
            <span className="text-lg font-black text-slate-900 tracking-tighter">
              Desde ${priceFinal.toLocaleString()}
            </span>
          ) : (
            <>
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        setProducts(data);
        setPagination(pag);
        if (slug) setCategoryName(data[0]?.category_name || slug.replace(/-/g, " "));
      } catch (err) {
          ...
          <div className="relative group w-full lg:w-96">
            ...
            {searchTerm && (
              <button
                ...
                className="absolute right-4 top-1/2 -translate-y-1/2 bg-slate-200 hover:bg-slate-300 p-1 rounded-full transition-all"
              >
                <X size={14} />
              </button>
            )}
          ...
          {products.length > 0 ? (
            products.map(p => (
              <ProductCard
                ...
                p={p}
                onToggle={toggleCart}
                // Para productos sin variante, verificamos cartKey base
            ...
            <div className="col-span-full py-32 flex flex-col items-center text-center">
              ...
              <Link
                to="/productos"
                onClick={() => setSearchTerm("")}
                className="bg-slate-900 text-white px-8 py-4 rounded-full font-bold mt-6 hover:bg-blue-600 transition-all active:scale-95 flex items-center gap-2"
              >
